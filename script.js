...
const CONFIG = {
  ...
  notas: {
    ...
    uber:   { valor: '4,5', tipo: 'sobre5',     pie: 'Top en Sevilla' },
    glovo:  { valor: '96',  tipo: 'porcentaje', pie: 'Valoración positiva' },
    google: { valor: '4,5', tipo: 'sobre5',     pie: 'Más de 300 reseñas' },
  },
...
(function () {
  ...
  function avisoFalta(que) {
    return (
      ...
      'Abre script.js y rellénalo en el bloque CONFIG de arriba.'
    );
  }
  ...
  if (btnUbi) {
    ...
    btnUbi.addEventListener('click', function () {
      ...
      decir('Tu navegador te va a pedir permiso para la ubicación.', 'cargando');
      ...
      navigator.geolocation.getCurrentPosition(
          ...
          if (cerca.km > LEJOS_KM) {
            ...
            decir(
              'Estás a ' + Math.round(cerca.km) + ' km de nuestro local más cercano. ' +
                'Desde ahí no te merece la pena venir: te mandamos una caja a casa.',
              'lejos'
            );
        ...
        function (err) {
          ...
          if (err.code === err.PERMISSION_DENIED) {
            msg = 'No nos has dado permiso, así que elige tu local aquí abajo.';
          } else if (err.code === err.POSITION_UNAVAILABLE) {
            msg = 'No hemos podido saber dónde estás. Elígelo tú aquí abajo.';
          } else {
            msg = 'Ha tardado demasiado. Elige tu local aquí abajo.';
          }
          decir(msg, 'error');
        },
...
try {
    ...
    if (guardado !== null && LOCALES[Number(guardado)]) {
      ...
      decir('Te hemos dejado el local de la última vez. Cámbialo si quieres.', 'ok');
    }
  } catch (e) {
  ...
  if (formRes) {
    ...
    formRes.addEventListener('submit', function (e) {
      ...
      if (pieRes) {
        pieRes.innerHTML = 'Te hemos dejado el correo redactado, solo tienes que enviarlo.' + telHtml;
        pieRes.className = 'f-foot es-ok';
      }
    });
  }
    ...
    formCat.addEventListener('submit', function (e) {
      ...
      fetch(CONFIG.cateringForm, {
        ...
        .catch(function () {
          ...
          botonCat.textContent = 'Pedir presupuesto';
          if (pie) {
            pie.innerHTML =
...
(function () {
  ...
  if (chipNombre && bpLocal) {
    const sincronizar = () => {
      ...
      bpLocal.textContent = n ? 'Recoger en ' + n : 'Elige tu local';
    };
    sincronizar();
...
(function () {
  ...
  video.play().then(
    ...
    function () {
      ...
      ctrl.querySelector('.visually-hidden').textContent = 'Reproducir el vídeo';
    }
  );
  ...
  if (formClub) {
    formClub.addEventListener('submit', function (e) {
      ...
      if (!CONFIG.clubForm) {
        ...
        if (pie) {
          pie.textContent = 'Se ha abierto tu correo con el alta escrita. Dale a enviar y te avisamos al abrir el club.';
      ...
      fetch(CONFIG.clubForm, {
        ...
        .catch(() => {
          ...
          boton.textContent = 'Quiero mis puntos';
          if (pie) {
            pie.innerHTML =
...
(function () {
  ...
  const PREGUNTAS = [
    { k: 'prote', t: '¿Con qué te apetece?', ops: [
      ['cerdo', 'Cerdo a fuego lento'], ['pollo', 'Pollo'],
      ['ternera', 'Ternera'], ['vegetal', 'Sin carne'],
    ]},
    { k: 'picante', t: '¿Y de picante, cómo vamos?', ops: [
      [0, 'Nada, gracias'], [1, 'Un puntito'], [2, 'Cuanto más mejor'],
    ]},
    { k: 'formato', t: 'Última: ¿cómo lo quieres?', ops: [
  ...
  async function resolver() {
    ...
    const motivos = [];
    if (gana.form === respuestas.formato) motivos.push('el formato que querías');
    if (gana.prote === respuestas.prote) motivos.push('la proteína que has pedido');
  ...
  async function reiniciar() {
    ...
    hilo.innerHTML = '';
    pie.innerHTML = '';
    burbuja('¡Órale! Dime tres cosas y te digo qué pedir.', 'ellos');
    await esperar(350);
    await preguntar();
  }
...
(function () {
  ...
  document.querySelectorAll('.nutri').forEach(function (ficha) {
    ...
    if (completos === 0) {
      ...
      if (nota) nota.textContent = 'Estamos calculándolos. Pregúntanos en el local.';
    } else if (nota) {
      nota.textContent = CONFIG.nutricion.estimado
        ? 'Valores estimados a partir de la receta, no medidos en laboratorio.'
        : 'Valores medios por ración. Pueden variar según el montaje.';
...
(function () {
  ...
  if (!CONFIG.pedidoPropio) return; // apagado: "Recoger" enlaza a Last App
